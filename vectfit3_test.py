...
def ex3():
	''' Fitting a measured admittance function from distribution transformer (single element)
	        ...
	        -3 iterations                                                                    '''
	f = np.empty((1,160), dtype=np.complex_)
	with open('03PK10.txt','r') as fl:
		next(fl) # skip the first lines
		next(fl)
		for k in range(160):
			A1,A2 = [float(a) for a in fl.readline().strip().split()]
			f[0,k] = A1 * np.exp(1j*A2*np.pi/180.0)

	w = 2.0 * np.pi * np.linspace(0,10e6,401)
	w = np.delete(w, 0)
	w = np.delete(w, np.s_[160:400])
	s = 1j * w
	Ns = s.shape[0]
